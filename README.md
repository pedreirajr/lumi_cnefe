...
This repository is **not an R package**. It is a regular R project with scripts you can source and run in your own workflow.
...
The code is part of a research project on mixed land use based on address data from the 2022 Brazilian census.
...
-   optional data files:
    -   `cnefe_index_2022.rds` – precomputed index of CNEFE 2022 URLs (can also be generated locally)
    -   `res_r7.rds`, `res_r8.rds`, `res_r9.rds` – precomputed results for selected cities and H3 resolutions
    -   `streets.rds` – precomputed arterial streets for the same cities (from OSM)

Some of these data files are large and may not be tracked by Git; they can be regenerated by running the scripts in this repository.
...
If you want to (re)download arterial streets from OpenStreetMap, you may also need:

-   `{osmdata}` or `{osmextract}`
